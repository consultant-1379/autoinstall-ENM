...
#chmod +x reset_passwords.bsh
#chmod +x reset_passwords.py
#sh /tmp/reset_passwords.bsh
whoami
...
cp /ericsson/deploymentDescriptions/2svc_enm_physical_test_dd.xml /tmp/logs/ms_logs/2svc_enm_physical_test_dd.xml
cp /var/log/audit/audit.log /tmp/logs/ms_logs/audit.log
...
do
    ...
    expect /tmp/scp_file.exp $hostnamei "/var/log/messages*" /tmp/logs/$hostnamei/
    ...
    do
        mv /tmp/logs/$hostnamei/$line1 /tmp/logs/$hostnamei/$line1.log
    done
    expect /tmp/scp_file.exp $hostnamei /var/log/litp/litp_libvirt.log /tmp/logs/$hostnamei/
    expect /tmp/scp_file.exp $hostnamei "/var/VRTSvcs/log/*" /tmp/logs/$hostnamei/
    expect /tmp/scp_file.exp $hostnamei "/etc/VRTSvcs/conf/config/main.cf*" /tmp/logs/$hostnamei/
    expect /tmp/scp_file.exp $hostnamei /var/log/mcollective.log /tmp/logs/$hostnamei/mcollective.log
    expect /tmp/scp_file.exp $hostnamei /var/log/mcollective-audit.log /tmp/logs/$hostnamei/mcollective-audit.log
    expect /tmp/scp_file.exp $hostnamei "/var/coredumps/*" /tmp/logs/$hostnamei/
    expect /tmp/scp_file.exp $hostnamei /var/log/boot.log /tmp/logs/$hostnamei/boot.log
done
echo ""
