...
def get_values_from_file(path_to_envvariables):
    ...
    global LITP_ISO, ENM_DROP, ENM_ISO, OS_PATCH_REV, OS_PATCH_ALPHA
    global SKIP_CLEAN, SKIP_OS_INSTALL, SKIP_PATCH_INSTALL, SKIP_LITP, SKIP_ENM
    global CI_VERSION, OS_RH7_PATCH_REV, OS_RH7_PATCH_ALPHA
    ...
    with open(path_to_envvariables, 'r') as infile:
        for line in infile:
            ...
            elif 'OSPatchRev' in line:
                OS_PATCH_REV = line.split("=")[1].rstrip()
            elif 'OSPatchAlpha' in line:
                OS_PATCH_ALPHA = line.split("=")[1].rstrip()
            elif 'RHEL7PatchRev' in line:
                OS_RH7_PATCH_REV = line.split("=")[1].rstrip()
            elif 'RHEL7PatchAlpha' in line:
                OS_RH7_PATCH_ALPHA = line.split("=")[1].rstrip()
...
def run_install():
    ...
    cmd = "/proj/lciadm100/cifwk/{12}/bin/cicmd deployment --clusterid {0}" \
          " --installType initial_install --environment physical --product " \
          "ENM --skipOsInstall {1} --osVersion 6.6_x86 --skipPatchInstall {2}"\
          " --osPatchVersion {3} --deployProduct LITP2 --skipLitpInstall {4} "\
          "--litpDrop {5}::{6} --skipEnmInstall {7} --drop {8}::{9} --setSED" \
          " {10} --xmlFile {11} --osRhel7PatchVersion={13}".format(CLUSTER_ID, SKIP_OS_INSTALL,
                SKIP_PATCH_INSTALL, OS_PATCH_REV, SKIP_LITP, LITP_DROP,
                    LITP_ISO, SKIP_ENM, ENM_DROP, ENM_ISO, SET_SED, XML_FILE, CI_VERSION, OS_RH7_PATCH_REV)
    ...
    if SKIP_PATCH_INSTALL == "NO":
        install_info += "\nOS Patches {0} (Rev {1})".format(
                OS_PATCH_REV, OS_PATCH_ALPHA)
    if SKIP_LITP == "NO":
