...
def get_values_from_file(path_to_envvariables):
    ...
    global CI_VERSION, OS_RH7_PATCH_VER, DEPLOY_PACKAGE, PRODUCT_SET
    global DEPLOY_LITP_PACKAGE
...
def run_install():
    ...
    cmd = "/proj/lciadm100/cifwk/{9}/bin/cicmd deployment --clusterid {0}" \
          ...
          "ENM --skipOsInstall {1} --osVersion 6.10_x86 --skipPatchInstall"\
          " {2} --deployProduct LITP2 --skipLitpInstall {3} "\
          "--litpDrop {4}::{5} --skipEnmInstall {6} --setSED" \
          " {7} --xmlFile {8} --productSet {10}".format(CLUSTER_ID,
                SKIP_OS_INSTALL, SKIP_PATCH_INSTALL, SKIP_LITP, LITP_DROP,
                LITP_ISO, SKIP_ENM, SET_SED, XML_FILE, CI_VERSION, PRODUCT_SET)
    ...
    if DEPLOY_PACKAGE != "":
        # If deployPackage is populated, add it to the install command
        cmd += " --deployPackage {0}".format(DEPLOY_PACKAGE)

    if DEPLOY_LITP_PACKAGE != "":
        # If deployLitpPackage is populated, add it to the install command
        cmd += " --deployLitpPackage {0}".format(DEPLOY_LITP_PACKAGE)
