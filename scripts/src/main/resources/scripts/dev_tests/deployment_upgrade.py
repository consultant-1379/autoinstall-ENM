...
def get_values_from_file(path_to_envvariables):
    ...
    global OS_PATCH_VER, SKIP_OS_PATCHES, CI_VERSION
    global OS_RH7_PATCH_VER, DEPLOY_PACKAGE, DEPLOY_LITP_PACKAGE
...
def run_upgrade():
    ...
    cmd = "/proj/lciadm100/cifwk/{0}/bin/cicmd deployment " \
          ...
          "--xmlFile {3} --productSet {4}".format(
              CI_VERSION, CLUSTER_ID, SET_SED, XML_FILE, PRODUCT_SET)
    ...
    if DEPLOY_PACKAGE != "":
        # If deployPackage is populated, add it to the upgrade command
        cmd += " --deployPackage {0}".format(DEPLOY_PACKAGE)	

    if DEPLOY_LITP_PACKAGE != "":
        # If deployLitpPackage is populated, add it to the upgrade command
